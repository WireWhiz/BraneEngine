...
template <typename T> std::string toHex(T num, bool keepLeadingZeros = false)
{
  if(num == 0)
    return "0";
  std::string hex;
...
template <typename T, size_t N> std::string toHex(std::array<T, N>& array, bool keepLeadingZeros = false)
{
  std::string hex;
...
template <typename T> std::string toHex(std::vector<T>& vector, bool keepLeadingZeros = false)
{
  std::string hex;
...
template <typename T> T fromHex(std::string_view hex)
{
  T num = 0;
  T pow = 1;
