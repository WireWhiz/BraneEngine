...
void Connection::addStreamListener(
    uint32_t id, std::function<void(InputSerializer s)> callback, std::function<void()> onEnd)
{
  _streamLock.lock();
...
template <> void ServerConnection<tcp_socket>::connectToClient()
{
  _address = _socket.remote_endpoint().address().to_string();
  async_readHeader();
}

template <> void ServerConnection<ssl_socket>::connectToClient()
{
...
template <>
void ClientConnection<tcp_socket>::connectToServer(
...
template <>
void ClientConnection<ssl_socket>::connectToServer(
    const asio::ip::tcp::resolver::results_type &endpoints,
    std::function<void()> onConnect,
    std::function<void()> onFail)
{
