...
ComponentID
ComponentManager::createComponent(const std::vector<VirtualType::Type> &component, const std::string &name) {
...
const ComponentDescription *ComponentManager::getComponentDef(ComponentID id) { return _components[id].get(); }
...
ComponentID ComponentManager::registerComponent(ComponentDescription *componentDescription) {
    ComponentID id =
            static_cast<ComponentID>(_components.push(std::unique_ptr<ComponentDescription>(componentDescription)));
    _components[id]->id = id;
