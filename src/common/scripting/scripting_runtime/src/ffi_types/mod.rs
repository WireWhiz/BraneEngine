...
impl<T: Clone> Clone for BEVec<T> {
    fn clone(&self) -> Self {
        let data = unsafe { Vec::from_raw_parts(self.data, self.length, self.capacity) };
...
impl From<&str> for BEStr {
    fn from(string: &str) -> Self {
        ...
        let new_string = BEStr{ data: c_str.into_raw() };
        new_string
    }
}
...
#[no_mangle]
pub extern "C" fn new_be_string(uninitalized: *mut BEStr) {
    unsafe { (*uninitalized).data = null_mut::<c_char>()};
}

#[no_mangle]
pub extern "C" fn free_be_string(string: *mut BEStr) {
    unsafe{
        drop(CString::from_raw((*string).data));
        (*string).data = null_mut();
    };
}

#[no_mangle]
pub extern "C" fn new_be_string_from_c_str(uninitalized: *mut BEStr, text: *const c_char) {
    let data = CString::from(unsafe { CStr::from_ptr(text) });
    unsafe {(*uninitalized).data = data.into_raw();};
}