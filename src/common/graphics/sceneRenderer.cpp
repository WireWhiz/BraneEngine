            ...
            _em.getEntities(filter).forEachNative([&](byte **components) {
                ...
                for (auto &mat: _vkr.materials()) {
                    ...
                    mat->bindUniformBuffer(
                            _swapChain.currentFrame(), 0, sizeof(RenderInfo),
                    ...
                    for (auto &renderObject: meshTransforms[mat->asset()->runtimeID]) {
                        ...
                        vkCmdPushConstants(
                                cmdBuffer, mat->pipelineLayout(), VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(uint32_t),
                                &instanceOffset);
                        ...
                        vkCmdBindIndexBuffer(
                                cmdBuffer, mesh->buffer(), mesh->indexBufferOffset(primitive),
                                mesh->indexBufferType(primitive));
                        vkCmdDrawIndexed(
                                cmdBuffer, static_cast<uint32_t>(mesh->indexCount(primitive)),
                                renderObject.second.size(), 0, 0, 0);
                    }
                }
