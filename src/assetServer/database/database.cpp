...
void Database::setAssetPermission(uint32_t assetID, uint32_t userID, AssetPermissionLevel level) {
    if (level != AssetPermissionLevel::none)
        _updateAssetPermission.run(static_cast<sqlINT>(assetID), static_cast<sqlINT>(userID),
                                   static_cast<sqlINT>(level));
    else
...
std::vector<Database::AssetSearchResult>
Database::searchAssets(int start, int count, std::string match, AssetType type) {
