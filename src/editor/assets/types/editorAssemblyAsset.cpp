...
public:
    UpdateEntityComponent(
            uint32_t entity, uint32_t componentIndex, Json::Value componentBefore, Json::Value component,
            VersionedJson *json)
            : JsonChange(
...
void EditorAssemblyAsset::updateEntityComponent(uint32_t entity, VirtualComponentView component, bool continuous) {
    ...
    if (continuous) {
        ...
        _json.recordChange(
                std::make_unique<UpdateEntityComponent>(entity, componentIndex, _componentBefore, newComponent,
                                                        &_json));
...
void
EditorAssemblyAsset::updateEntityComponent(uint32_t entity, uint32_t component, Json::Value value, bool continuous) {
    if (continuous) {
        if (_componentBefore.isNull())
...
public:
    AddEntityComponent(uint32_t entity, Json::Value component, VersionedJson *json)
            : JsonArrayChange("entities/" + std::to_string(entity) + "/components", 0, std::move(component), true,
                              json) {
        _entity = entity;
    }
...
void EditorAssemblyAsset::changeMaterial(uint32_t materialIndex, const AssetID &materialID) {
    if (materialID.null())
        ...
        Runtime::getModule<AssetManager>()->fetchAsset<Asset>(materialID).then(
                [this, materialIndex, materialID](Asset *m) {
                    _json.recordChange(std::make_unique<MaterialChange>(materialIndex, materialID, &_json));
                });
}
