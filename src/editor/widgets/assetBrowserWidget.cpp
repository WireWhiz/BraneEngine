        ...
        if (enter || ImGui::Button("create")) {
            ...
            switch (_type.type()) {
                ...
                case AssetType::shader:
                    auto *shader = new EditorShaderAsset(_widget.currentDirectory() / (_assetName + ".shader"),
                                                         editor->project());
...
void AssetBrowserWidget::displayDirectoriesRecursive(FileManager::Directory *dir) {
    ...
    bool nodeOpen = ImGui::TreeNodeEx(
            (((dir->open && !dir->children.empty()) ? ICON_FA_FOLDER_OPEN : ICON_FA_FOLDER) + dir->name).c_str(),
            flags);
    dir->open = nodeOpen;
...
void AssetBrowserWidget::displayFiles() {
    ...
    if (!_currentDir)
        ...
        for (size_t i = 0; i < _contents.size(); ++i) {
            ...
            if (ImGui::Selectable(
                    "##", isSelected, ImGuiSelectableFlags_SelectOnRelease | ImGuiSelectableFlags_AllowDoubleClick,
                    {0, 0})) {
                if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
                    if (type == FileType::asset && _allowEdits) {
                        ...
                        std::shared_ptr<EditorAsset> asset = editor->project().getEditorAsset(
                                currentDirectory() / file);
                        if (asset)
                            _ui.sendEvent(std::make_unique<FocusAssetEvent>(asset));
                    }
                    if (type == FileType::directory) {
                        // Since directories are sorted to the top of files and in the same manner as directory children, they have
                        // the same index
            ...
            if (_selectedFiles.x != -1) {
                ...
                if (_selectedFiles.x == _selectedFiles.y) {
                    if (getFileType(_contents[_selectedFiles.x]) == FileType::source &&
                        ImGui::Selectable(ICON_FA_UP_RIGHT_FROM_SQUARE
                        " Edit")) {
#ifdef _WIN32
