...
class CreateDirectoryPopup : public GUIPopup {
  ...
  void drawBody() override
  {
    ...
    bool enter =
        ImGui::InputText("##name", &_dirName, ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_EnterReturnsTrue);
...
void AssetBrowserWidget::displayFiles()
{
  ...
  if(!_currentDir)
    ...
    for(size_t i = 0; i < _contents.size(); ++i) {
      ...
      if(ImGui::Selectable(
             "##", isSelected, ImGuiSelectableFlags_SelectOnRelease | ImGuiSelectableFlags_AllowDoubleClick, {0, 0})) {
