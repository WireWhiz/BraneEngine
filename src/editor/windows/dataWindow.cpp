        ...
        if (_search.draw()) {
            ...
            Runtime::getModule<AssetManager>()
                    ...
                    .then([this, asset, entity](ComponentAsset *component) {
                        ...
                        _focusedAsset->addEntityComponent(_focusedEntity,
                                                          EditorAssemblyAsset::componentToJson(newComp));
                    });
    ...
    for (Json::ArrayIndex i = 0; i < entityAsset["components"].size(); ++i) {
        ...
        if(ImGui::BeginDragDropTarget())
        {
            if(const ImGuiPayload* p = ImGui::AcceptDragDropPayload("component"))
            {
                ...
                // The min is to account for if we're trying to move the component to the end, and just removed it from the
        same list. destComponents.insert(destComponents.begin() + std::min<uint32_t>(destComponents.size(), i), source);
            }
...
void DataWindow::displayImageData() {
    ...
    if (_imagePreview) {
        ...
        if (texture)
            _imagePreview =
                    ImGui_ImplVulkan_AddTexture(texture->sampler(), texture->view(),
