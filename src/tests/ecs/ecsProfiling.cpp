...
TEST(ECS_Profiling, QueryCreation_WorstCase)
{
  ...
  std::cout << "Running test with " << components.size() + 1 << " components and " << allSets.size() << " archetypes"
            << std::endl;
  ...
  SystemContext ctx;
  {
    ...
    std::cout << smallResult1 << " nanoseconds to find " << smallSet1.archetypeCount()
...
TEST(ECS_Profiling, QueryCreation_PlausableCase)
{
  ...
  std::cout << "Running test with " << components.size() + 1 << " components and " << randomSets.size() << " archetypes"
            << std::endl;
