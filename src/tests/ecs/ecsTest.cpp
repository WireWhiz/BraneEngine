...
em.
addComponent(entity, vcd3
.id);
em.
addComponent(entity, vcd1
.id);
em.
removeComponent(entity, vcd2
.id);
em.
removeComponent(entity, vcd3
.id);
em.
removeComponent(entity, vcd1
.id);
...
4);
EXPECT_EQ(em
._archetypes._archetypes[0].

size(),

1);
EXPECT_EQ(em
._archetypes._archetypes[1].

size(),

0);
EXPECT_EQ(em
._archetypes._archetypes[2].

size(),

0);
EXPECT_EQ(em
._archetypes._archetypes[3].
...
em.
addComponent(e, TestNativeComponent::def()
->id);
// Create a new archetype
if(i > 49)
em.
addComponent(e, TestNativeComponent2::def()
->id);
entities[i] =
e;
}
...
std::cout << "For Each on 50 entities took: " << time <<
std::endl;

std::cout << "Test done, checking values..." <<
std::endl;
for(
size_t i = 0;
i < 100; i++) {
...
EXPECT_EQ(true, ts1->var1) << "entity: " <<
i;
EXPECT_EQ(74, ts1->var2) << "entity: " <<
i;
EXPECT_EQ(74, ts1->var3) << "entity: " <<
i;
EXPECT_EQ(true, ts2->var1) << "entity: " <<
i;
}
else {
// Entites with one
TestNativeComponent *ts1 = em.getComponent<TestNativeComponent>(entities[i]);
EXPECT_EQ(false, ts1->var1) << "entity: " <<
i;
EXPECT_EQ(32, ts1->var2) << "entity: " <<
i;
EXPECT_EQ(42, ts1->var3) << "entity: " <<
i;
}
}
